...
def test_speed_voiced_frame():
    assert calculate_frame_duration(frame_data["voiced-1"]["data"],frame_data["voiced-1"]["fs"]) == 0.04
    
def test_speed_unvoiced_frame():
    assert calculate_frame_duration(frame_data["unvoiced-1"]["data"],frame_data["unvoiced-1"]["fs"]) == 0.04
    
def test_speed_noise_frame():
    assert calculate_frame_duration(frame_data["noise-1"]["data"],frame_data["noise-1"]["fs"]) == 0.04
    
def test_speed_empty_frame():
    assert calculate_frame_duration([],48000) == 0
    
def test_pitch_voiced():
    assert calculate_frame_pitch(frame_data["voiced-1"]["data"],frame_data["voiced-1"]["fs"]) == pytest.approx(115.64, 0.01)
    
def test_pitch_unvoiced():
    assert math.isnan(calculate_frame_pitch(frame_data["unvoiced-1"]["data"],frame_data["unvoiced-1"]["fs"]))
    
def test_pitch_noise():
    assert math.isnan(calculate_frame_pitch(frame_data["noise-1"]["data"],frame_data["noise-1"]["fs"]))
...
def test_formants_unvoiced_frame():
    formants = calculate_frame_f1_f2(frame_data["unvoiced-1"]["data"],frame_data["unvoiced-1"]["fs"])
    assert len(formants)==2
