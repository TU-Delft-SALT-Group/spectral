...
def test_speed_voiced_frame():
    assert (
        calculate_frame_duration(array("h", frame_data["voiced-1"]["data"]), frame_data["voiced-1"]["fs"]) == 0.04
    ), "Expected duration for voiced frame to be 0.04 seconds"


def test_speed_unvoiced_frame():
    assert (
        calculate_frame_duration(array("h", frame_data["unvoiced-1"]["data"]), frame_data["unvoiced-1"]["fs"]) == 0.04
    ), "Expected duration for unvoiced frame to be 0.04 seconds"


def test_speed_noise_frame():
    assert (
        calculate_frame_duration(array("h", frame_data["noise-1"]["data"]), frame_data["noise-1"]["fs"]) == 0.04
    ), "Expected duration for noise frame to be 0.04 seconds"


def test_speed_empty_frame():
    assert calculate_frame_duration(array("h", []), 48000) == 0, "Expected duration for empty frame to be 0 seconds"


def test_pitch_voiced():
    assert calculate_frame_pitch(
        array("h", frame_data["voiced-1"]["data"]), frame_data["voiced-1"]["fs"]
    ) == pytest.approx(115.64, 0.01), "Expected pitch for voiced frame to be approximately 115.64 Hz"


def test_pitch_unvoiced():
    assert math.isnan(
        calculate_frame_pitch(array("h", frame_data["unvoiced-1"]["data"]), frame_data["unvoiced-1"]["fs"])
    )


def test_pitch_noise():
    assert math.isnan(
        calculate_frame_pitch(array("h", frame_data["noise-1"]["data"]), frame_data["noise-1"]["fs"])
    ), "Expected pitch for noise frame to be NaN"


def test_pitch_empty_frame():
    assert math.isnan(calculate_frame_pitch(array("h", []), 48000)), "Expected pitch for empty frame to be NaN"


def test_formants_voiced_frame():
    formants = calculate_frame_f1_f2(array("h", frame_data["voiced-1"]["data"]), frame_data["voiced-1"]["fs"])
...
def test_formants_unvoiced_frame():
    formants = calculate_frame_f1_f2(array("h", frame_data["unvoiced-1"]["data"]), frame_data["unvoiced-1"]["fs"])
...
def test_formants_noise_frame():
    formants = calculate_frame_f1_f2(array("h", frame_data["noise-1"]["data"]), frame_data["noise-1"]["fs"])
